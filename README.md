...
- `src/device_info.cu` / `src/cuda_device_info.cu` – 设备信息探测工具，输出 SM 数、L1/L2 cache、显存、时钟等关键硬件数据，便于在跑基准前验证测试 GPU。
...
1. **Device info** – 先调用 `device_info` / `cuda_device_info` 打印当前 GPU 的 SM 数、L1/L2、显存与时钟，确认测试目标是否正确。
2. **FP64 compute** – four kernel shapes covering 256–768 blocks × 256–512 threads with 4K–8K inner-loop iterations，覆盖更高占用率与指令混合。
3. **Memory bandwidth** – H2D/D2H/D2D 传输覆盖 1MB 到 1GB 以 2 的幂递增的多个尺寸（1、2、4、8、16、32、64、128、256、512、1024 MB），并额外给出 float4 向量化 `copy_kernel`（读+写）计算出的 “RW Copy GB/s”（若驱动不支持 PTX，则自动退化为 2×D2D 结果并提示升级建议），更细致地观察 cache/TLB 与分页影响。
4. **hcBLAS / cuBLAS GEMM** – 同时测试 FP64、FP32、FP16、TF32（FP16/TF32 走 GEMMEx/Compute32F+Tensor Core），覆盖 1024³ 到 16384³（及多个矩形尺寸）的多种矩阵形状，并重复 20 次以稳定计时。
5. **Vector add (FP32/FP64)** – 1M～64M 元素的 `C=A+B`，以 80 次循环求平均，考察访存绑定的简单逐元素算子。
6. **Matrix add (FP32/FP64)** – 2048²～8192² 及 4096×8192 的元素级加法，衡量更大 2D 布局的吞吐能力。
...
# Device info
build/bin/device_info              # hc / MXGPU
build/bin/cuda_device_info --device 0   # NVIDIA
```
